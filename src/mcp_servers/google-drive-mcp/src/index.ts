    ...
    tools: [
      {
        ...
        inputSchema: {
          type: "object",
          properties: {
            query: { type: "string", description: "Google Drive API query (e.g., \"name contains 'report'\" or \"fullText contains 'budget'\"). For simple search, use \"fullText contains 'term'\"." },
            ...
            slides: {
              ...
              items: {
                ...
                properties: {
                  ...
                  content: { type: "string", description: "Slide content (optional)" },
                  slideLayout: { type: "string", description: "Layout (e.g. TITLE, TITLE_AND_BODY, BLANK). Default: TITLE_AND_BODY", optional: true }
                }
              }
            },
            parentFolderId: { type: "string", description: "Parent folder ID (defaults to root)", optional: true }
  ...
  try {
    switch (request.params.name) {
      case "search": {
        ...
        // Use the query as-is, but ensure we filter out trashed items if not specified
        let formattedQuery = userQuery;
        if (!formattedQuery.toLowerCase().includes("trashed")) {
          formattedQuery = `(${formattedQuery}) and trashed = false`;
        }
      ...
      case "createGoogleSlides": {
        ...
        for (const slide of args.slides) {
            ...
            requestBody: {
              requests: [{
                createSlide: {
                  ...
                  slideLayoutReference: { predefinedLayout: slide.slideLayout || 'TITLE_AND_BODY' },
                }
              }]
            },
          });
          ...
          if (slide.title || slide.content) {
            ...
            const requests: any[] = [];
            if (slide.title && titlePlaceholderId) {
              requests.push({ insertText: { objectId: titlePlaceholderId, text: slide.title, insertionIndex: 0 } });
            }
            if (slide.content && bodyPlaceholderId) {
              requests.push({ insertText: { objectId: bodyPlaceholderId, text: slide.content, insertionIndex: 0 } });
            }
